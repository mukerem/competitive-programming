...
def auth(username, password):
    ...
    response = session.post(auth_url, data=login)
    print(response.text)
    if not response.status_code == 200:
        raise Exception("Invalid credintials")

def get_detail(session, link):
    url = f"https://open.kattis.com{link}"
    data = {"script": "true"}
    page_content = session.get(url, data=data)
    ...
    code = session.get(url, data=data)
...
with requests.Session() as session:
    ...
    download_submission(session, username)
